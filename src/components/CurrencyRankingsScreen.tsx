...
import { buildNationalCurrencyRows } from "../utils/currencyRankingsShared";
...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onCountrySelect?: (cca2: string) => void;
};

export const CurrencyRankingsScreen: React.FC<Props> = ({ isOpen, onClose, onCountrySelect }) => {
  ...
  const rankingMeta = useMemo(() => {
    ...
    const fallbackExplanation =
      !hasConfigured && configuredRef !== "USD"
        ? `No spot rate for ${configuredRef} in today’s dataset — showing USD equivalents.`
        : null;
    const rows = buildNationalCurrencyRows(ratesTable, effectiveRefIso);
    ...
    return rows
      ...
      .filter(
        (r) =>
          ...
          || (r.cca2?.toLowerCase().includes(q) ?? false),
      );
  }, [rows, search]);
      ...
      <div className="stat-sheet">
        <div className="stat-header">
          ...
          <div className="stat-header-center">
            ...
            <div>
              <div className="stat-header-title">Currency rankings</div>
              <div className="stat-header-desc">
                {ratesTable === null && !err
                  ? "Loading rates…"
                  : err
                    ?? `National currencies · per 1 unit in ${effectiveRefIso} (${refCountryLabel}) · ${frankfurterTodayDisplayLabel()}`}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="cry-ref-summary">
            <div className="cry-ref-chip">
              ...
              <div className="cry-ref-text">
                <div className="cry-ref-label">Your reference</div>
        ...
        <div className="stat-search-bar">
          ...
          <span className="stat-count-badge">
            {rows === null ? "…" : search ? `${visibleRows.length} / ${rows.length}` : `${rows.length} currencies`}
          </span>
        </div>

        <div className="stat-list">
          {rows === null ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--text-secondary)" }}>Fetching latest rates…</div>
          ) : err ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--text-secondary)" }}>{err}</div>
          ) : visibleRows.length === 0 ? (
            <div className="stat-no-results">{`No matches for "${search}"`}</div>
          ) : (
            visibleRows.map((row) => (
              <div
                key={row.currency}
                className="stat-row"
                role={row.cca2 && onCountrySelect ? "button" : undefined}
                tabIndex={row.cca2 && onCountrySelect ? 0 : undefined}
                onClick={() => {
                  if (row.cca2) onCountrySelect?.(row.cca2);
                }}
                onKeyDown={(e) => {
                  if (!row.cca2 || !onCountrySelect) return;
                  if (e.key === "Enter" || e.key === " ") {
                    e.preventDefault();
                    onCountrySelect(row.cca2);
                  }
                }}
                style={{ cursor: row.cca2 && onCountrySelect ? "pointer" : "default" }}
              >
